...
public class PaintApi implements Api {
     ...
     * Const pool.
     */
    public static final String DRAW_EASY_FUNCTION = "drawefunc\\s+-[a-z]{2,6}\\s+\\d+\\s+\\d+\\s+\\d+\\s+\\d+\\s+\\d+\\s+\\d+";
    ...
    public static final String JAVA_S_S = "java\\s+[^\\s]+";
    ...
    private void analysis (String command) {
        ...
        //if java
        else if (command.matches(JAVA_S_S)) {
            String path = command.split("\\s+")[1];
            Jcc jcc = new Jcc(path);
            try {
                jcc.read();
                jcc.compile();
                jcc.write();
                call = "Export script to java file[" + jcc.get() + "]";
            } catch (IOException | CompilerSyntaxErrorException e) {
                call = e.getLocalizedMessage();
            }
            call += "\nPixPainter>";
            jcc.clear();
        ...
        //if new a script and edit it.
        else if (command.matches("new\\s+.+\\s+.+")) {
            String[] params = command.split("\\s+");
            new FileEditor(params[1], params[2] + ".kScript");
            call = "";
            return;
        }
        //if create image
        if (command.matches(CREATE_REGEX)) {
            ...
            if (g2D == null) {
                ...
                -atan/-sqrt/-cube/-exp/-log/-ln/-square] [x1] [x2] [x] [y] [w] [my]
                 */
                    ...
                    if (EASY_FUNCTION_POOL.contains(params[1])) {
                        if (params.length == 8) {
                            ...
                            int w = Integer.parseInt(params[6]);
                            int my = Integer.parseInt(params[7]);
                            setDrawEasyFunction(params[1], new Range(x1, x2), x, y, w, my);
                            call += "function range is[" + x1 + ", " + x2 + "]";
                        } else {
                            call = "Illegal input";
                        }
                        call += "\nPixPainter>";
 ...
 */
    ...
    final void setDrawEasyFunction (String function, Range range, int x, int y, double width, double dy) {
        GeneralPath gp = new GeneralPath();
        float size = (float) (width / (range.getX2() - range.getX1()));
        gp.moveTo(x, y);
        switch (function) {
            case "-sin": {
                for (int i : range) {
                    double yPos = y - (float) (dy * sin(PI * i / 180));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            }
            case "-cos": {
                for (int i : range) {
                    double yPos = y - (float) (dy * cos(PI * i / 180));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-tan": {
                for (int i : range) {
                    double yPos = y - (float) (dy * tan(PI * i / 180));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-csc": {
                for (int i : range) {
                    double yPos = y - (float) (dy * (1 / sin(PI * i / 180)));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-sec": {
                for (int i : range) {
                    double yPos = y - (float) (dy * (1 / cos(PI * i / 180)));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-cot": {
                for (int i : range) {
                    double yPos = y - (float) (dy * (1 / tan(PI * i / 180)));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-asin": {
                for (int i : range) {
                    double yPos = y - (float) (dy * asin(PI * i / 180));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-atan": {
                for (int i : range) {
                    double yPos = y - (float) (dy * atan(PI * i / 180));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
            ...
            case "-log": {
                for (int i : range) {
                    double yPos = y - (float) (dy * log10(i));
                    gp.lineTo(i * size, yPos);
                }
                g2D.draw(gp);
                break;
        ...
        call = "Execute and draw the function:" + function.replaceFirst("^-", "") + " x";
        Logger.Builder.set("./Log");
        Logger logger = Logger.Builder.build();
        logger.log(LogType.EVENT, call);
        call += "\nPixPainter>";
    ...
    final String pre (String command) {
        return command.
                replaceFirst("^[a-z-A-Z]+\\s*", command.split("\\s+")[0].
                        toLowerCase() + " ");
    }
